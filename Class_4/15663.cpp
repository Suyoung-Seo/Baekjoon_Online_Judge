...
void dfs(int N, int M, int depth, int *arr, bool *visited, int *num, set<vector<int>> *st)
{
    ...
    else
    {
        for (int i = 1; i <= N; i++)
        {
            if (visited[i] == false) // 숫자를 방문하지 않았을 경우
            {
                ...
                dfs(N, M, depth + 1, arr, visited, num, st); // 다음 경우의 수 dfs 탐색
